...
def run():
    ...
    rss_writer = RSSWriter(
        rss_path,
        "newsletter must read",
        "https://study.tczhong.com",
        "news letter for tech worlds",
    )
    rs = []
    ...
    for feed in rs:
        rss_writer.add_feed(
            feed["title"], feed["link"], feed["content"], feed["publishTime"]
        )
    rss_writer.write_data("newsletter.xml", "")
    s3.write_data("news", os.path.join(rss_path, "newsletter.xml"))
