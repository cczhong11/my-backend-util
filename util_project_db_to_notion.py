...
class ProjectNotionPush(NotionPush):
    ...
    def push_data(self, data: NotionItem, dst: str):
        new_page_props = {
            ...
            "Language": {
                "type": "multi_select",
                "multi_select": [{"name": data.language}],
            }
            if data.language
            else notion_util("unknown", "multi_select"),
            "Last Update": notion_util(data.last_update, "date")
...
db = SqliteDBReader(
    "/Users/tianchenzhong/Dropbox/TCCode/ML/project_classifier/db.sqlite"
)
