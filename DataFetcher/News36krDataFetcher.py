...
class News36krDataFetcher(DataFetcherBase):
    ...
    def extract_content(self, link):
        ...
        res = res[0]
        res = json.loads(res)
        return res.get("data", {}).get("widgetContent",""), res.get("data", {}).get("publishTime",0)
    ...
    def process_topic(self, link):
        ...
        for feed in feedList:
            ...
            rs.append({"title": title, "content": content, "link": f"https://36kr.com{link}","publishTime": publishTime})
        return rs
