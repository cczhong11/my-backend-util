...
class DoubanFetcher(DataFetcherBase):
    ...
    def get_data(self, query):
        feedRes = {}
        result = requests.get(
            f"https://www.douban.com/search?cat={CAT_ID[self.topic]}&q={query}",
            headers=self.header,
        ).content
        ...
        if len(feedList) == 0:
            return {}
        feed = feedList[0]
        onclick = feed.findAll("a")[0].get("onclick")
        sid = re.findall(r"sid: (\d+)", onclick)[0]
    ...
    def health_check(self):
        result = requests.get(
            "https://www.douban.com/search?cat=1002&q=tt10327252", headers=self.header
        ).content
        soup = BeautifulSoup(result, "lxml")
        feedList = soup.findAll("div", attrs={"class": "content"})
