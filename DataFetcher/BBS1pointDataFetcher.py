...
class BBS1pointDataFetcher(DataFetcherBase):
    ...
    def get_data(self, url=""):
        ...
        for idx, feed in enumerate(self.feedList):
            ...
            if (
                feed.findChild("td", attrs={"class": "by"}) is None
                or INTERVAL not in feed.findChild("td", attrs={"class": "by"}).text
            ):
                continue
    ...
    def health_check(self):
        ...
        self.feedList = soup.findAll(
            "tbody", id=lambda x: x and x.startswith("normalthread_")
        )
